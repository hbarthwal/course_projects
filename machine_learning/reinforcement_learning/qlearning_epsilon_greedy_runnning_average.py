...
class State:
    ...
    def _getEpsilonGreedyRandomAction(self):
        randomNum = randint(1, 100) / float(100)
        if randomNum < self.epsilon:
            return self._getRandomAction()
        else:
            return self._getMaxQValueAction()
    ...
    def _getMaxQValueAction(self):
        sortedValues = sorted(self.qValuesDict.items(), None , itemgetter(1) , True)
        return sortedValues[0][0]
    
    def getStochasticAction(self):
        self.intendedAction = self._getEpsilonGreedyRandomAction()
    ...
    def calculateExpectedReward(self):
        for action in self.actionRewardDict:
            if (self.name == 'S3' or self.name == 'S5' or self.name == 'S9') and self.visitsDict[action] != 0:
                print 'E[r(', self.name,',',action,')] = ', self.actionRewardDict[action] * self.goalVisitDict[action] / float(self.visitsDict[action])  
